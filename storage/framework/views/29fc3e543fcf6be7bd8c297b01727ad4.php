...
<body class="bg-gray-100 text-gray-800">
    ...
    <header class="fixed top-0 left-0 right-0 bg-white shadow-sm z-50">
        <div class="container mx-auto px-4 py-3 flex items-center justify-between">
            ...
            <nav class="hidden md:flex items-center space-x-6">
                <a href="<?php echo e(route('admin.dashboard')); ?>" class="text-gray-900 hover:text-primary font-medium text-sm">Beranda</a>
                <a href="<?php echo e(route('admin.dokumen.show', ['id' => 1])); ?>" class="text-gray-600 hover:text-primary font-medium text-sm">Dokumen</a>
  ...
  <section class="flex justify-between px-8 mt-6 mb-2 space-x-4">
    <div class="bg-white rounded-xl shadow p-4 w-1/5 flex items-center justify-between">
      <div>
        ...
        <p class="text-2xl font-bold"><?php echo e($documents->count()); ?></p>
      </div>
    ...
    <div class="bg-white rounded-xl shadow p-4 w-1/5 flex items-center justify-between">
      <div>
        ...
        <p class="text-2xl font-bold text-yellow-500">
          <?php echo e($documents->where('status', 'Menunggu')->count()); ?>
    ...
    <div class="bg-white rounded-xl shadow p-4 w-1/5 flex items-center justify-between">
      <div>
        ...
        <p class="text-2xl font-bold text-green-500">
          <?php echo e($documents->where('status', 'Disetujui')->count()); ?>
    ...
    <div class="bg-white rounded-xl shadow p-4 w-1/5 flex items-center justify-between">
      <div>
        ...
        <p class="text-2xl font-bold text-red-500">
          <?php echo e($documents->where('status', 'Ditolak')->count()); ?>
  ...
  <main class="px-8 py-2">
    <form method="GET" action="<?php echo e(url('/manajemen')); ?>"
    ...
    <div class="bg-white rounded-xl shadow overflow-x-auto">
      <table class="min-w-full text-sm">
        ...
        <tbody>
          <?php $__currentLoopData = $documents; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $doc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <tr class="border-t">
              <td class="p-3">
                <input type="checkbox" name="selected[]" value="<?php echo e($doc->id); ?>" form="bulk-action-form" />
              </td>
              ...
              <td class="p-3"><?php echo e($doc->user->username ?? '-'); ?></td>
              <td class="p-3"><?php echo e($doc->created_at->format('d-m-Y')); ?></td>
              <td class="p-3">
                <?php
                  $statusColors = [
                ...
                ?>
                <span class="<?php echo e($colorClass); ?> px-2 py-1 rounded-full text-xs"><?php echo e($doc->status ?? '-'); ?></span>
              </td>
              <td class="p-3 flex gap-2">
                <?php if($doc->status == 'Menunggu'): ?>
                  <form action="<?php echo e(url('/manajemen/' . $doc->id . '/approve')); ?>" method="POST" class="inline">
                    <?php echo csrf_field(); ?>
                  ...
                  <form action="<?php echo e(url('/manajemen/' . $doc->id . '/reject')); ?>" method="POST" class="inline">
                    <?php echo csrf_field(); ?>
                ...
                <form action="<?php echo e(route('manajemen.destroy', $doc->id)); ?>" method="POST" class="inline"
      ...
      <div class="px-4 py-2 border-t border-gray-200 flex justify-between items-center text-sm">
        <form id="bulk-action-form" method="POST" action="<?php echo e(url('/manajemen/bulk-action')); ?>" class="flex gap-2">
          <?php echo csrf_field(); ?>
        ...
        <div class="flex justify-end items-center gap-2 p-4">
          <a href="<?php echo e($page > 1 ? url('/manajemen?page=' . ($page - 1)) : '#'); ?>"
             class="px-3 py-1 border rounded <?php echo e($page <= 1 ? 'text-gray-400 cursor-not-allowed' : 'hover:bg-gray-200'); ?>">
              &laquo;
          </a>
          <?php for($i = 1; $i <= $totalPages; $i++): ?>
              <a href="<?php echo e(url('/manajemen?page=' . $i)); ?>"
                 class="px-3 py-1 border rounded <?php echo e($page == $i ? 'bg-blue-600 text-white' : 'hover:bg-gray-200'); ?>">
                  <?php echo e($i); ?>

              </a>
          <?php endfor; ?>
          <a href="<?php echo e($page < $totalPages ? url('/manajemen?page=' . ($page + 1)) : '#'); ?>"
             class="px-3 py-1 border rounded <?php echo e($page >= $totalPages ? 'text-gray-400 cursor-not-allowed' : 'hover:bg-gray-200'); ?>">
              &raquo;
          </a>
        </div>
      </div>
...
<?php /**PATH C:\xampp\htdocs\Pemrograman Web\coretax-lite-restore-laravel10\coretax-lite-restore-laravel10\resources\views/manajemen.blade.php ENDPATH**/ ?>